...
class StatePublisher(object):
	def __init__(self):
		...
		# rospy.Subscriber("/novatel/oem7/inspva",INSPVA, self._parse_gps_vel, queue_size=1)
		# rospy.Subscriber('/gps/imu', Imu, self._parse_imu_data, queue_size=1)
		# rospy.Subscriber('/steering_angle', Float32Msg, self._parse_steering_angle, queue_size=1)
		...
		rospy.Subscriber('/current_steer', Int32, self._parse_steering_angle, queue_size=1)
		######################
		...
		while not rospy.is_shutdown():		
			...
			if time_check_on:
				time_valid = is_time_valid(extract_ros_time(curr_state), [self.tm_gps, self.tm_vel, self.tm_imu, self.tm_df])
				if not time_valid:
					r.sleep()
					continue
    ...
    # curr_time = data.header.stamp.to_sec()
		...
		self.lat = data.pose.position.x
		self.lon = data.pose.position.y
		self.x, self.y = latlon_to_XY(self.LAT0, self.LON0, self.lat, self.lon)
		...
		self.tm_gps = extract_ros_time(data)
	...
	def _parse_steering_angle(self, msg):
		...
		# self.df = m.radians(msg.data) / 15.87 # steering angle, delta_f (rad)
		self.df = m.radians(msg.data)/10.57 # steering angle, delta_f (rad)
		# print("sssssssssssssssssssssssssss: ",msg)


		# self.tm_df = extract_ros_time(msg)
		self.tm_df = time.time()
